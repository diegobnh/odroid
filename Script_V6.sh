...
if [ -d "~/diego/result" ];then     
   mkdir ~/diego/result
else
   rm ~/diego/result/*.out; rm ~/diego/result/*.txt
fi 
...
if [ $FLAG_SPLASH3 == 'True' ]; then
	for tf in `echo $THREAD_FACTOR`
	  ...
	  for ((i = 0; i < ${#CPU_LIST[@]}; i++));
		  ...
		  then
		      mv ~/diego/result/perf.out ~/diego/result/"perf_${APP_SPLASH3_NAMES[$j]}.${CPU_LIST[$i]}.txt"
		  else
		  ...
		  then
		      mv ~/diego/result/time.out ~/diego/result/"time_${APP_SPLASH3_NAMES[$j]}.${CPU_LIST[$i]}.txt"
		  else
