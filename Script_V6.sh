...
PATH=/home/odroid/diego

if [ -d "$PATH/result/" ];then     
   mkdir $PATH/result
else
   rm $PATH/result/*.out; rm $PATH/result/*.txt
fi 


PERF_COMMAND="perf stat -o ~/diego/result/perf.out -I 200 -e instructions,cycles,cache-misses,cache-references,branches,branch-misses"
TIME_COMMAND="/usr/bin/time -p -o ~/diego/result/time.out"
	  ...
	  for ((i = 0; i < ${#CPU_LIST[@]}; i++));
          do             
			...
			0) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; 
                           ...
                           then $TIME_COMMAND $PERF_COMMAND ./contiguous_partitions/OCEAN -p ${NUM_THREADS[$i]} -n 2050 & pid_app=$!
                           fi ;;

			1) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND $PERF_COMMAND ./RADIOSITY -p ${NUM_THREADS[$i]} -room -batch & pid_app=$!;;

			2) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND $PERF_COMMAND ./RAYTRACE -p${NUM_THREADS[$i]} -a1600 -m100 inputs/car.env & pid_app=$!;;

			3) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND $PERF_COMMAND ./VOLREND ${NUM_THREADS[$i]} inputs/head 512 & pid_app=$!;;		

			4) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND $PERF_COMMAND ./contiguous_blocks/LU -n4096 -p${NUM_THREADS[$i]} -b512 & pid_app=$!;;

			5) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND $PERF_COMMAND ./non_contiguous_blocks/LU -n4096 -p${NUM_THREADS[$i]} -b512 & pid_app=$!;;

			6) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; 
                           if [[ "${NUM_THREADS[$i]}" == "1" || "${NUM_THREADS[$i]}" == "2" || "${NUM_THREADS[$i]}" == "4" || "${NUM_THREADS[$i]}" == "8" ]]
                           then $TIME_COMMAND $PERF_COMMAND ./RADIX -p${NUM_THREADS[$i]} -n134217728 -r4096 & pid_app=$!
                           fi ;;
			...
			*) echo "INVALID NUMBER!" ;;
	          ...
	          #perf stat --per-thread -o /tmp/perf.out -I 200 -p $pid_app & pid_perf=$!
           	  ...
           	  wait $pid_app
                  ...
                  if [ -f "~/diego/result/perf.out" ]
		  then
		      mv $PATH/result/perf.out $PATH/result/"perf_${APP_SPLASH3_NAMES[$j]}.${CPU_LIST[$i]}.txt"
		  else
		      printf "\n !!! Perf didnt create perf.out to ${APP_SPLASH3_NAMES[$j]} !!! \n"
                  fi

                  if [ -f "~/diego/result/time.out" ]
		  then
		      mv $PATH/result/time.out $PATH/result/"time_${APP_SPLASH3_NAMES[$j]}.${CPU_LIST[$i]}.txt"
		  else
