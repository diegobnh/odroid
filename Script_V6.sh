	  ...
	  for ((i = 0; i < ${#CPU_LIST[@]}; i++));
          do             
			...
			0) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; 
                           ...
                           then $TIME_COMMAND taskset -a -c ${CPU_LIST[$i]} $PERF_COMMAND ./contiguous_partitions/OCEAN -p ${NUM_THREADS[$i]} -n 2050 
                           fi ;;

#			1) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND taskset -a -c ${CPU_LIST[$i]} $PERF_COMMAND ./RADIOSITY -p ${NUM_THREADS[$i]} -room -batch;;

#			2) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND taskset -a -c ${CPU_LIST[$i]} $PERF_COMMAND ./RAYTRACE -p${NUM_THREADS[$i]} -a1600 -m100 inputs/car.env ;;

#			3) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND taskset -a -c ${CPU_LIST[$i]} $PERF_COMMAND ./VOLREND ${NUM_THREADS[$i]} inputs/head 512 ;;		

#			4) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND taskset -a -c ${CPU_LIST[$i]} $PERF_COMMAND ./contiguous_blocks/LU -n4096 -p${NUM_THREADS[$i]} -b512 ;;

#			5) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; $TIME_COMMAND taskset -a -c ${CPU_LIST[$i]} $PERF_COMMAND ./non_contiguous_blocks/LU -n4096 -p${NUM_THREADS[$i]} -b512;;

#			6) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]} -- Config:${CPU_LIST[$i]}\n\n"; 
 #                          if [[ "${NUM_THREADS[$i]}" == "1" || "${NUM_THREADS[$i]}" == "2" || "${NUM_THREADS[$i]}" == "4" || "${NUM_THREADS[$i]}" == "8" ]]
  #                         then $TIME_COMMAND taskset -a -c ${CPU_LIST[$i]} $PERF_COMMAND ./RADIX -p${NUM_THREADS[$i]} -n134217728 -r4096 
   #                        fi ;;
