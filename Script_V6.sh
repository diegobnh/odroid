...
TIME_COMMAND="/usr/bin/time -p -o /tmp/perf.out/time.out"
...
if [ $FLAG_SPLASH3 == 'True' ]; then
	for tf in `echo $THREAD_FACTOR`
	  ...
	  for ((i = 0; i < ${#CPU_LIST[@]}; i++));
		  ...
		  then
		      mv /tmp/perf.out/perf.out /tmp/perf.out/"perf_${APP_SPLASH3_NAMES[$j]}.${CPU_LIST[$i]}.txt"
		  else
		  ...
		  then
		      mv /tmp/perf.out/time.out /tmp/perf.out/"time_${APP_SPLASH3_NAMES[$j]}.${CPU_LIST[$i]}.txt"
		  else
