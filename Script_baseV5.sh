...
PERF_COMMAND="perf stat -o /tmp/perf.out -e cycles,instructions,cache-misses,LLC-load-misses"
...
APP_PARSEC3_NAMES=("apps/blackscholes" "apps/bodytrack" "apps/facesim" "apps/ferret" "apps/fluidanimate" "apps/freqmine" "apps/raytrace" "apps/swaptions" "apps/x264" "apps/vips"  "kernels/canneal" "kernels/dedup" "kernels/streamcluster")
          ...
          do             
             ...
             for ((j = 0; j < ${#APP_PARSEC3[@]}; j++)); do
	          cd  ${APP_PARSEC[$j]} #Path specific benchmark
                  ...
                  #all simlarge input:
                  case $j in
			0) printf "\n-- Running blackscholes--\n\n"; $PERF_COMMAND ./blackscholes $NUM_THREADS in_64K.txt /tmp/temp.out & pid_app=$!;;

			1) printf "\n-- Running bodytrack--\n\n"; $PERF_COMMAND ./bodytrack sequenceB_4/ 4 1 1000 3 0 $NUM_THREADS & pid_app=$!;;

			2) printf "\n-- Running facesim--\n\n"; $PERF_COMMAND ./facesim -timing -threads $NUM_THREADS & pid_app=$!;;

			3) printf "\n-- Running ferret--\n\n"; $PERF_COMMAND ./ferret corel lsh queries 10 20 $NUM_THREADS output.txt & pid_app=$!;;

			4) printf "\n-- Running fluidanimate--\n\n"; $PERF_COMMAND ./fluidanimate $NUM_THREADS 5 ../../../inputs/in_300K.fluid out.fluid & pid_app=$!;;

			5) printf "\n-- Running freqmine--\n\n"; export OMP_NUM_THREADS=$NTHREADS; $PERF_COMMAND ./freqmine ../../../inputs/kosarak_990k.dat 790 & pid_app=$!;;

			6) printf "\n-- Running raytrace--\n\n"; $PERF_COMMAND ./raytrace rtview happy_buddha.obj -automove -nthreads $NUM_THREADS -frames 3 -res 1920 1080 & pid_app=$!;;

			7) printf "\n-- Running swaptions--\n\n"; $PERF_COMMAND ./swaptions -ns 64 -sm 40000 -nt $NUM_THREADS & pid_app=$!;;

			8) printf "\n-- Running x264--\n\n"; $PERF_COMMAND ./x264 --quiet --qp 20 --partitions b8x8,i4x4 --ref 5 --direct auto --b-pyramid --weightb --mixed-refs --no-fast-pskip --me umh --subme 7 --analyse b8x8,i4x4 --threads $NUM_THREADS -o eledream.264 ../../../inputs/eledream_640x360_128.y4m & pid_app=$!;;

			9) printf "\n-- Running vips--\n\n"; export IM_CONCURRENCY=$NTHREADS; $PERF_COMMAND ./vips im_benchmark bigben_2662x5500.v output.v & pid_app=$!;;

			10) printf "\n-- Running canneal--\n\n"; $PERF_COMMAND ./canneal $NUM_THREADS 15000 2000 400000.nets 32 & pid_app=$!;;

			11) printf "\n-- Running dedup--\n\n"; $PERF_COMMAND ./dedup -c -p -v -t $NUM_THREADS -i media.dat -o output.dat.ddp & pid_app=$!;;

			12) printf "\n-- Running streamcluster--\n\n"; $PERF_COMMAND ./streamcluster 10 20 128 16384 16384 1000 none output.txt $NUM_THREADS & pid_app=$!;;
