...
APP_SPLASH3=("apps/barnes" "apps/fmm" "apps/ocean" "apps/radiosity" "apps/raytrace" "apps/volrend" "apps/water-nsquared" "apps/water-spatial" "kernels/cholesky" "kernels/ftt" "kernels/lu" "kernels/lu" "kernels/radix")

APP_SPLASH3_NAMES=("barnes" "fmm" "ocean contiguous" "radiosity" "raytrace" "volrend" "water-nsquared" "water-spatial" "cholesky" "fft" "lu contiguous" "lu nocontiguous" "radix")
          ...
          do             
             ...
             for ((j = 0; j < ${#APP_SPLASH3[@]}; j++)); do
	          cd  ${APP_SPLASH3[$j]} #Path specific benchmark
                  ...
                  #all simlarge input:
                  case $j in
			0) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./BARNES inputs/n16384 -p $NUM_THREADS & pid_app=$!;;

			1) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./FMM inputs/"input"$NUM_THREADS"16384" & pid_app=$!;;

			2) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./contiguous_partitions/OCEAN -p $NUM_THREADS -n 258 & pid_app=$!;;

			3) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./RADIOSITY -p $NUM_THREADS -ae 5000 -bf 0.1 -en 0.05 -room -batch & pid_app=$!;;

			4) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./RAYTRACE -p $NUM_THREADS -m64 inputs/car.env & pid_app=$!;;

			5) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./VOLREND $NUM_THREADS inputs/head 8 & pid_app=$!;;

			6) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./WATER-NSQUARED inputs/n512-p$NUM_THREADS & pid_app=$!;;

			7) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./WATER-SPATIAL inputs/n512-p$NUM_THREADS & pid_app=$!;;

			8) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./CHOLESKY -p $NUM_THREADS inputs/tk15.O & pid_app=$!;;

			9) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./FFT -p $NUM_THREADS -m 16 & pid_app=$!;;

			10) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./contiguous_blocks/LU -p $NUM_THREADS -n 512 & pid_app=$!;;

			11) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./non_contiguous_blocks/LU -p $NUM_THREADS -n 512 & pid_app=$!;;

			12) printf "\n-- Running ${APP_SPLASH3_NAMES[$j]}--\n\n"; ./RADIX -p $NUM_THREADS -n 1048576 & pid_app=$!;;
			...
			*) echo "INVALID NUMBER!" ;;
	          ...
	          #perf stat --per-thread -o /tmp/perf.out -I 200 -p $pid_app & pid_perf=$!
           	  ...
           	  wait $pid_app
                  ...
                  if [ -f "/tmp/perf.out" ]
		  then
		      mv /tmp/perf.out /tmp/"perf_${APP_SPLASH3_NAMES[$j]}.txt"
		  else
