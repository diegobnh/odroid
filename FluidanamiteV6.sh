	  ...
	  for ((i = 0; i < ${#CPU_LIST[@]}; i++));
          do             
			...
			0) printf "\n-- Running fluidanimate-- Config:${CPU_LIST[$i]}\n\n"; 
                           ...
                           then $TIME_COMMAND taskset -a -c ${CPU_LIST[$i]} perf stat -o /tmp/perf.out -e instructions,cycles,cache-misses,cache-references,branches,branch-misses ./fluidanimate ${NUM_THREADS[$i]} 500 ../../../inputs/in_500K.fluid out.fluid 
                           fi ;;
