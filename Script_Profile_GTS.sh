...
echo "PID ./run-nqueens="$pid_app

sleep 1
NewPid=`pgrep -f nqueens.gcc.omp`
echo "Novo PID="$NewPid
#pid=$(pgrep nqueens.gcc.omp)
#echo "Novo PID="$pid
...
./Freq.sh & pid_freq=$!
echo "PID FREQ="$pid_freq
./Status.sh $NewPid & pid_status=$!
echo "PID STATUS="$pid_status
./Mapping.sh $NewPid & pid_mapping=$!
echo "PID MAPPING="$pid_mapping
